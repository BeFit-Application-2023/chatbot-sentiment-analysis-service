    ...
    def execute(self) -> None:
        ...
        while True:
            ...
            if self.priority_queue.qsize() > 0:
                ...
                pred = self.model(torch.stack([embeds]))[0]
                pred.detach()
                probabilities = torch.softmax(pred, dim=0).detach().numpy()

                score = 0 * probabilities[0] + 0.5 * probabilities[1] + 1 * probabilities[2]
                task.prediction = score
